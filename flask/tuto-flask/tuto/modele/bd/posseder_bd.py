"""
    Ce fichier  va nous permettre de crée la class posseder liée à la bd
"""
import sys
import os
...
class Posseder_bd:
    ...
    def get_posseder_by_idinteret(self, idinteret):
        """
            ...
            param idinteret: ID de l'intérêt pour lequel on souhaite récupérer
            les associations de possession.
            return: Une liste d'objets Posseder représentant les associations de
            possession liées à l'intérêt donné.
        """
        try:
            query = text("select * from POSSEDER where id_interet = " +
                         str(idinteret))
            resultat = self.cnx.execute(query)
            possession = []
            for ide, idi in resultat:
    ...
    def get_posseder_by_etape(self, idetape):
        """
            ...
            param idetape: ID de l'étape pour laquelle on souhaite
            récupérer les associations de possession.
            return: Une liste d'objets Posseder représentant les
            associations de possession liées à l'étape donnée.
        """
        try:
            query = text("select * from POSSEDER where id_etape = " +
                         str(idetape))
            resultat = self.cnx.execute(query)
            possession = []
            for ide, idi in resultat:
    ...
    def inserer_possede(self, idetape, idinteret):
        ...
        try:
            query = text(
                f"insert into POSSEDER values({str(idetape)} , {str(idinteret)})"
            )
