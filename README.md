...
- Pour lancer l'application, assurez-vous d'installer tous les modules en exécutant simplement le script bash nommé `installation_module.sh`
- Une fois cela réalisé, si vous êtes sur les PC fixe présent à l'IUT, vous n'aurez qu'à vous rendre au chemin suivant (en partant de la racine du projet) `cd flask/tuto-flask`  et exécuter la commande `flask run`.
  - Si cela ne fonctionne pas, malgré que vous avez exécuté le script installation fourni, vous pouvez essayer cette commande qui a fonctionné pour les membres de notre groupe : `~/.local/bin/flask run`
- Puis vous pouvez profiter de l'application !
...
**Rappel** : Ces équipes ne sont pas fixes et, au fur et à mesure que le projet avance, les rôles de chacun en sont influencés.
...
## **Travail effectué lors de la seconde semaine**:
...
## **Travail effectué lors de la troisième semaine**:
